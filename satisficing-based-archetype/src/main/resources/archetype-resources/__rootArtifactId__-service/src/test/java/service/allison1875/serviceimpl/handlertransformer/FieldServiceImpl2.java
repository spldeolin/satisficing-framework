...
#set( $symbol_dollar = '$' )
#set( $symbol_escape = '\' )
package ${package}.service.allison1875.serviceimpl.handlertransformer;
        ...
        if (Lists.newArrayList("Date", "LocalDate", "LocalTime", "LocalDateTime")
            ...
            if (!field.getAnnotationByName("JsonFormat").isPresent()) {
                field.addAnnotation(StaticJavaParser.parseAnnotation(
                        "@com.fasterxml.jackson.annotation.JsonFormat(pattern = ${symbol_escape}"yyyy-MM-dd HH:mm:ss${symbol_escape}", timezone = "
                                + "${symbol_escape}"Asia/Shanghai${symbol_escape}")"));
            }
        }
